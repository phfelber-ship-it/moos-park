...
type RoomImage = { url: string; pathname: string };
...
export default function RoomImageManager({
  ...
  const saveOrder = async (next: RoomImage[]) => {
    setImages(next);
    setSavingOrder(true);
    try {
      const res = await fetch("/api/admin/room-images/order", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ slug, order: next.map((i) => i.pathname) }),
      });
  ...
  const migrate = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/admin/room-images/migrate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ slug }),
      });
      if (!res.ok) throw new Error("Übernahme fehlgeschlagen.");
      const data = (await res.json()) as { uploaded: RoomImage[] };
      ...
      {images.length === 0 ? (
        <>
          <p className="mt-8 text-sm text-foreground/50">
            Noch keine Bilder im Verwaltungs-Speicher für diesen Raum -
            aktuell laufen die urspruenglichen Standardbilder (Vorschau
            unten, schreibgeschuetzt).
          </p>
